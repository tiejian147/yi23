...
*           数组：字面意思是"一组数据"
*       3、数组当中可以存储"基本数据类型"的数据，也可以存储"引用数据类型的数据
*       4、数组因为是引用类型，所以数组对象是堆内存当中（数组是存储在堆当中）
*       5、数组当中如果存储的是"java对象"的话，实际上存储的是对象的"引用（内存地址）"
*       6、数组一旦创建，在java中规定，长度不可变（数组长度不可变）
*       7、数组的分类，一维数组，二维数组，三维数组，多维数组...（一维数组较多，二维数组偶尔使用！）
*       8、所有的数组对象都有length属性（java自带的），用来获取数组中元素的个数。
*       9、java中的数组要求数组中元素的类型统一，比如int类型数组只能存储int类型，Person类型的数组只能存储Person类型。
*       10、数组在内存方面存储的时候，数组中的元素内存地址（存储的每一个元素都是有规则的挨着排列的）是连续的，内存地址连
...
*                       出某个下表上元素的内存地址，直接通过内存地址定位元素，所以数组的检索效率是最高的。
*
*                   数组中存储100个元素，或者存储100万个元素，在元素查询/检索方面，效率是相同的，因为数组中元素查找的时候不会一个
*                   一个找，是通过数学表达式计算出来的（算出一个内存地址，直接定位的）
*
*           缺点：
*               第一：由于为了保证数组中每个元素的内存地址连续，所以在数组上随机删除或者增加元素的时候，
*                    效率较低，因为随机你增删元素会涉及到后面元素统一向前或者向后位移的操作。
*               第二：数组不能存储大数据量，为什么？
...
//        下标为5表示第六个元素，第六个元素没有，下标越界了，会有什么异常？
//        编译报错：ArrayIndexOutOfBoundsException（比较著名的异常）
