...
链表的优点 ：
    ...
    所以随机增删元素的时候不会有大量元素位移，因为随机增删效率较高。
    在以后的开发中，如果遇到随机增删集合中元素的业务比较多时，建议
    使用Linklist。

链表的缺点：
    不能通过数学表达式计算被查找元素的内存地址，每一次查找都是从头节点
    开始遍历，知道找到为止，所以Linklist集合检索/查找的效率较低。
...
//        因为只能从头结点开始一个一个遍历。
        ...
        不管是LinkedList 还是ArrayList，以后写代码时不需要关心具体是哪个集合。
        因为我们要面向接口编程，调用的方法都是接口中的方法。
         */

//        这样写表示底层你用了数组
//        List list2 = new ArrayList();//这行代码也可以从 数组集合对象直接改成

//        这样写表示底层用了双向链表
        List list2 = new LinkedList();//双向链表集合对象
